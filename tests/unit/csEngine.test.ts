...
import { BombRound, BOMB_RULES } from '../../src/games/csBombMode.js';
import { BUY_CATEGORIES, SHOP, awardMoney, settleRound } from '../../src/games/csEconomy.js';
import { MAPS } from '../../src/games/csMaps.js';
import { RECOIL, KEY_ACTIONS } from '../../src/games/csWeaponBehavior.js';
import { DEPLOY } from '../../src/games/csWeaponDeploy.js';
import { SnowWorld } from '../../src/games/csWorld.js';
import { WEAPONS, SOLDIER_HITBOX } from '../../src/games/csWeapons.js';

const ASSET_ROOT = join(process.cwd(), 'public/cs');
...
describe('cs engine: weapon data', () => {
  it('every weapon has coherent stats, recoil and deploy data', () => {
    for (const [id, def] of Object.entries(WEAPONS) as [string, any][]) {
      ...
      expect(DEPLOY[id], `${id} deploy`).toBeTruthy();
      expect(DEPLOY[id].duration, `${id} deploy duration`).toBeGreaterThan(0);
      ...
      expect(RECOIL[id], `${id} recoil`).toBeTruthy();
    }
  });
...
describe('cs engine: economy', () => {
  it('every shop item maps to a category and a real weapon or equipment id', () => {
    ...
    for (const item of SHOP) {
      ...
      expect(WEAPONS[item.id], item.id).toBeTruthy();
    }
  });
