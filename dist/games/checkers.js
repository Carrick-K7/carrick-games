...
export class CheckersGame extends BaseGame {
    ...
    init() {
        this.board = Array.from({ length: BOARD }, () => Array(BOARD).fill(0));
    ...
    getPieceMoves(c, r, player, board = this.board) {
        ...
        const dirs = [];
        if (isPlayer || isKing)
            dirs.push([-1, -1], [1, -1]); // up-left, up-right (player goes up)
        if ((!isPlayer && player === 2) || isKing)
    ...
    getAllMoves(player, board = this.board) {
        const all = [];
        for (let c = 0; c < 8; c++) {
            for (let r = 0; r < 8; r++) {
                const p = board[c][r];
    ...
    aiTurn() {
        ...
        const move = candidates.reduce((best, m) => {
            if (m.captures.length > best.captures.length)
                return m;
            if (m.captures.length < best.captures.length)
                return best;
    ...
    executeMove(move) {
        ...
        // King promotion
        let newPiece = piece;
        if (piece === 1 && move.toR === 0)
            newPiece = 3;
        if (piece === 2 && move.toR === 7)
            newPiece = 4;
    ...
    handleInput(e) {
        ...
        const bx = MARGIN;
        const by = MARGIN;
        if (x < bx || y < by || x >= bx + BOARD * CELL || y >= by + BOARD * CELL)
            return;
