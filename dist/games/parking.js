...
export class ParkingGame extends BaseGame {
    constructor() {
        ...
        this.car = { x: 0, y: 0, angle: 0, vx: 0, vy: 0, steerAngle: 0 };
        this.levelIndex = 0;
        this.score = 0;
        this.elapsed = 0;
        this.timeLeft = 0;
    ...
    carCollidesRect(rx, ry, rw, rh) {
        ...
        for (const c of corners) {
            if (this.pointInRect(c.x, c.y, rx, ry, rw, rh))
                return true;
        }
    ...
    checkCollisions() {
        for (const obs of this.level.obstacles) {
            if (this.carCollidesRect(obs.x, obs.y, obs.w, obs.h))
                return true;
        }
        return false;
    }
    ...
    update(dt) {
        if (this.gameState === 'crash' || this.gameState === 'timeout' || this.gameState === 'complete')
            return;
        ...
        if (speed > 5) {
            if (left)
                this.car.angle -= this.TURN_SPEED * dt * Math.sign(speed > 0 ? 1 : -1);
            if (right)
                this.car.angle += this.TURN_SPEED * dt * Math.sign(speed > 0 ? 1 : -1);
        }
        // Friction
