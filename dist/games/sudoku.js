...
export class SudokuGame extends BaseGame {
    ...
    generatePuzzle() {
        ...
        const clues = this.difficulty === 'easy'
        ...
        const puzzle = full.map((r) => r.map((v) => ({
            value: v,
            given: true,
            notes: new Set(),
            invalid: false,
    ...
    draw(ctx) {
        ...
        ctx.textAlign = 'right';
        ctx.fillText(`${zh ? '错误' : 'Mistakes'} ${this.mistakes}/${this.maxMistakes}`, this.width - 16, infoY);
        ctx.fillText(`${zh ? '提示' : 'Hints'} ${this.hints}/${this.maxHints}`, this.width - 16, infoY + 16);
        // Grid
        const gridSize = 432;
    ...
    drawMenu(ctx, isDark, text, primary, secondary, zh) {
        const difficulties = [
    ...
    drawNumpad(ctx, x, y, width, height, isDark, text, primary, secondary, zh) {
        const cols = 9;
