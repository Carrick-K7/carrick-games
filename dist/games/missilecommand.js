...
export class MissileCommandGame extends BaseGame {
    ...
    update(dt) {
        if (this.state === 'waiting' || this.gameOver)
            return;
    ...
    draw(ctx) {
        ...
        if (this.state === 'waiting') {
            this.drawOverlay(ctx, isDark, 'CLICK TO LAUNCH', zh ? '点击发射导弹' : 'CLICK TO LAUNCH');
        }
        else if (this.state === 'gameover') {
            this.drawOverlay(ctx, isDark, `GAME OVER  SCORE: ${this.score}`, zh ? `游戏结束  得分: ${this.score}` : `GAME OVER  SCORE: ${this.score}`);
        }
        else if (this.state === 'waveclear') {
            this.drawOverlay(ctx, isDark, `${zh ? '波' : 'WAVE'} ${this.wave - 1} ${zh ? '完成' : 'CLEAR'}!`, `${zh ? `第${this.wave - 1}波完成` : `WAVE ${this.wave - 1} CLEAR`} ${zh ? '波即将开始' : 'incoming'}`);
        }
    }
    drawOverlay(ctx, isDark, text, textZh) {
...
//# sourceMappingURL=missilecommand.js.map