...
export class SimonGame extends BaseGame {
    ...
    getColorFromPoint(clientX, clientY) {
    ...
    drawStat(ctx, x, y, label, value, theme, align) {
        ctx.textAlign = align;
    ...
    drawOverlay(ctx, theme, zh) {
        ...
        ctx.fillText(this.phase === 'ready' ? (zh ? '记忆游戏' : 'SIMON SAYS') : (zh ? '游戏结束' : 'GAME OVER'), W / 2, 222);
        ctx.fillStyle = theme.muted;
        ctx.font = '8px "Press Start 2P", "Noto Sans SC", sans-serif';
        if (this.phase === 'ready') {
    ...
    drawCard(ctx, x, y, w, h, radius, fill, stroke) {
        ctx.beginPath();
