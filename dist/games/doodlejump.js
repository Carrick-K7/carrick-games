...
export class DoodleJumpGame extends BaseGame {
    ...
    addPlatform(y) {
        const types = ['normal', 'normal', 'normal', 'moving', 'fragile', 'disappearing'];
        const type = types[Math.floor(Math.random() * types.length)];
        const pw = PLATFORM_W;
        const px = Math.random() * (W - pw);
        const p = { x: px, y, w: pw, type, alpha: 1, broken: false };
        if (type === 'moving')
            p.dx = (Math.random() < 0.5 ? 1 : -1) * (80 + Math.random() * 60);
        return p;
    }
    startGame() {
    ...
    update(dt) {
        ...
        // Horizontal movement
        let vx = 0;
        if (this.keys.left || this.touchSide === 'left')
            vx = -MOVE_SPEED;
        if (this.keys.right || this.touchSide === 'right')
            vx = MOVE_SPEED;
        this.player.vx = vx;
        this.player.x += this.player.vx * dt;
        // Screen wrap
        if (this.player.x + CHAR_W < 0)
            this.player.x = W;
        if (this.player.x > W)
            this.player.x = -CHAR_W;
        // Gravity
...
//# sourceMappingURL=doodlejump.js.map