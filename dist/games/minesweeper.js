...
export class MinesweeperGame extends BaseGame {
    constructor() {
        ...
        this.grid = []; // -1=mine, 0-8=adjacent count
        this.revealed = [];
        this.flagged = [];
        this.gameState = 'idle';
            ...
            for (let x = 0; x < COLS; x++) {
                ...
                for (let dy = -1; dy <= 1; dy++) {
                    for (let dx = -1; dx <= 1; dx++) {
                        const ny = y + dy;
                        const nx = x + dx;
                        if (ny >= 0 && ny < ROWS && nx >= 0 && nx < COLS && this.grid[ny][nx] === -1)
                            count++;
                    }
                }
                this.grid[y][x] = count;
    ...
    handleInput(e) {
        ...
        if (isKeyDown) {
            ...
            if (e.key === 'ArrowRight' || e.key === 'd')
                this.cursorX = Math.min(COLS - 1, this.cursorX + 1);
            if (e.key === 'ArrowUp' || e.key === 'w')
                this.cursorY = Math.max(0, this.cursorY - 1);
            if (e.key === 'ArrowDown' || e.key === 's')
                this.cursorY = Math.min(ROWS - 1, this.cursorY + 1);
            if (e.key === ' ' || e.key === 'Enter') {
                if (this.gameState === 'idle')
                    this.gameState = 'playing';
                if (this.gameState === 'playing')
                    this.revealCell(this.cursorX, this.cursorY);
                this.timer = 0;
    ...
    update(dt) {
        ...
        if (this.gameState === 'won' || this.gameState === 'lost') {
            if (!this._recorded) {
                this._recorded = true;
                window.reportScore?.(this.gameState === 'won' ? Math.max(0, Math.floor(10000 - this.timer * 10)) : 0);
            }
        }
    }
    draw(ctx) {
