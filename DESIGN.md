...
- **Renovated plan:** the ground-floor study is a tea room with three usable floor cushions; the east lounge stays unfurnished, with storage in the garage. Gym/snooker/lounge boundaries are open; cinema retains separate acoustic treatment and single/double/three-place seating. Upstairs, the former family room is open hall; its contents are in the east reading hall, and the north-east activity room is a massage room. The guest room occupies the north-west strip, dressing/ensuite the middle strip, and the master gallery wall is at x=-4. Joinery must leave every doorway and window clear, with physical colliders matching the fitted objects. The main bathroom has exactly two operable privacy-glass doorways (west/east), live swept door collision, a 50%-larger tub and a separate shower. Roof landscaping follows the outer perimeter, with three independently hinged grills; the unused half of the top stair mouth has a solid glass safety panel.
- **Audit evidence:** `games/villa/tests/map-audit.mjs` captures real game frames to `/tmp/carrick-villa-audit` by default, never root-level test files. Every capture forces a fresh bounded software-GL render and checks the actual camera/frame counter; changing a HUD label over a cached image is not visual evidence. Geometry, fitted-room traversal, door interlocks, seats, roads, audio lifecycle and rendering budgets have separate regression tests.
