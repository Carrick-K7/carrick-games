...
- Exactly **three permanent shell actions**, each a real native target rather than a larger visual pseudo-element around a smaller hitbox:
  ...
  |---|---|---|
  | Carrick Games / `#siteBrand` | 44×44px; exact 22px gamepad, icon-only (the stacked wordmark appears on the `…` menu and library) | Top/left 12px for fixed games; top 12px/right 116px for responsive CS, CS Kimi and Villa |
...
- **Renovated plan:** the ground-floor study is a tea room with three usable floor cushions; the east lounge stays unfurnished, with storage in the garage. Gym/snooker/lounge boundaries are open; cinema retains separate acoustic treatment and single/double/three-place seating. Upstairs, the former family room is open hall; its contents are in the east reading hall, and the north-east activity room is a massage room. The guest room occupies the north-west strip, dressing/ensuite the middle strip, and the master gallery wall is at x=-4. Joinery must leave every doorway and window clear, with physical colliders matching the fitted objects. The main bathroom has exactly two operable privacy-glass doorways (west/east): double-track telescoping panels slide north in 1.5 seconds, with real jamb/centre overlaps that also protect oblique privacy rays, and swept-body interlocks. Keep the 50%-larger tub and separate shower. Roof landscaping follows the outer perimeter, with three independently hinged grills; the unused half of the top stair mouth has a solid glass safety panel.
- **Audit evidence:** `games/villa/tests/map-audit.mjs` captures real game frames to the ignored `games/villa/test-results/map-audit` by default, never root-level test files. Capture both standing exterior views and actual seated cockpit views, and distinguish their support checks. Every capture forces a fresh bounded software-GL render and checks the actual camera/frame counter; changing a HUD label over a cached image is not visual evidence. Geometry, fitted-room traversal, door interlocks, seats, roads, audio lifecycle and rendering budgets have separate regression tests.
...
- **Access and safety:** the native mobile Use/Exit hit target exactly covers its canvas circle, fires once, hides with overlays/immersive mode and is removed on cleanup. Rejected actions explain why in up to two readable mobile lines, and a genuine refusal to a player action appears as a short-lived contextual message at the occupied object rather than a permanent footer. Crouching, gravity-driven jumps and head clearance preserve real floor/shaft/pool support. Stair finish and structure never share a plane. The elevator travels continuously through real slab openings with interlocked doors; 1/2/3 selects a floor inside. Occupancy or an obstructed sill prevents closure, while an empty cabin closes after four seconds. The staircase and the lift swapped places (`STAIR_HOLE` west, shaft east at `x=4.55`, doors facing +Z): keep both, keep the switchback stair continuous at 16.7°, and never replace travel with teleportation. Only the driver's door opens on each of the three road vehicles (`E` opens, seats and closes on one action); the car panel lists the top floor first and has its own Open/Close buttons.
- **Orientation and estate:** +Z is south/front, +X east; the entrance faces south and the four-bay garage is east. The envelope is authored as four datums (`VILLA_WEST_WALL`, `VILLA_EAST_WALL`, `VILLA_NORTH_WALL`, `VILLA_SOUTH_WALL`) and every wall, slab, room, lamp, camera, fence and map extent derives from them, so the plan can be resized without hunting literals. In 1.3.0 the interior doubled west, east and north around a fixed stair-and-lift core, which now sits near the middle of the plan, and 1.4.0 divided the resulting long bands on a single `z=-9` line so no room runs more than about 18 m; the garage moved east with the house so the ground floor and the upper storeys share one footprint. The lawn mesh is cut away under the whole building and pool footprints, because a flat lawn sharing a plane with the interior slabs z-fights at thresholds and makes the lift floor look grassy. Preserve the planted driving loop, west pool, garden aisles, roof terrace and southern rolling roads/fields/pond. The estate now extends north to z=-58: a 2–3m meandering stream near z=-38 shares exact bank/water polygons with its lawn cutout and hazards; the timber crossing has continuous 3m ramps and actual supported deck. Raw lawn height must not include the deck, or grass will cover the bridge. All vehicles, walking, rain and map outlines consume the same contract. Regional mixed copses enrich all four sides without adding obstacles to roads or a hedge wall. Fruit/leaf geometry distinguishes ten fruit trees; rooftop blooms and four vegetable beds remain botanical rather than labelled boxes or a crop-management game. The pool's water, basin, ground cutout and supports share the `POOL` datum, with loungers on its south cedar deck anchored to that same datum rather than to a remembered coordinate.
- **Tea and aquarium:** align actual BACK edges to the fireplace chimney datum `z=-0.31` (chimney centre `-0.1`, depth `0.42`), not unlike-depth counter centres. Tea bar `x=-7.05`, tank `x=-3.98`; preserve cabinet gaps, the `x=-2` room wall, living-side approaches and live collider bounds, including pulls. The visible tea cup starts empty; E progressively brews for ten seconds, a full/ready cup persists, then E lifts/tilts it, depletes liquid and sets it down empty. Busy presses do not reset brewing/drinking. Keep physical utensils, subtle steam and the independently operable recessed kitchen tap; no score/flood mechanic. The tank has ten slender top-view ornamental medaka with a blue-silver dorsal stripe, upturned mouth and rear-set translucent fins. Shrimp have been removed at the user's request; do not restore them. No fat goldfish stand-ins.
- **Seats, beds and wardrobes:** maintain 36 authored house/roof/pool seating/rest definitions covering sofas, dining and roof chairs, PC/guest chairs, stools, loungers and both beds, plus the outdoor swing and relocatable camping chair. Resolve sofa positions along their actual cushion segments from the pre-seat position; do not force one entry side. Small seats stay on their cushions. Preserve free look, correct floor/yaw, own-seat collider identity and collision-checked entry/exit candidates. Each bed has one centred pillow and a lying pose with the head nearby. The five-bay master wardrobe stays on the solid north partition, clear of glazing and doors; ten separated animated leaves expose nine women's compartments and only the rightmost small men's compartment, with ordinary garments, hats, bags and shoes. Update every moving collider, avoid coplanar panels and do not hide the open contents behind an opaque cabinet box. Bedroom poses and furniture share `villaBedroomLayout.ts`: the guest bed sits on the south-west solid wall, far from both entrances; both bedrooms have purposeful sleeping, sitting and writing/dressing zones. The 3.2m master vanity is against the north-west solid wall, with open knee space, a sittable stool, cosmetics and a 2.5m framed mirror without a reflection render target. Keep real circulation around the low master divider and through the balcony door.
...
- **Vehicles and rally:** locally authored continuous coachwork distinguishes the pearl electric fastback, utility pickup with a real open bed, and blue/cognac SUV. Opaque crowned roofs, fitted glazing, wheel arches, curved fascias and front-facing lamps must agree with their hollow cockpits and declared envelopes; interior trim may not protrude through tapered outer skins. Exterior tint remains visibly glazed on software GL while preserving the seated road view. Four garage bays hold three cars and one reserved bay; keep workshop props and charging pedestal outside through routes. One contextual driver-door action opens, safely boards/exits and closes the door; motion, blocked corridors and lifecycle cancellation remain guarded. Road sedan/SUV/pickup/scooter W accelerates, S first brakes then reverses from rest, A/D steers and Space supplies a separate handbrake. Manual road-car limits are 180/160/140km/h for sedan/SUV/pickup, not the former 25km/h limiter; speed-sensitive steering and spatially bounded collision substeps accompany the increase. Automatic parking deliberately stays slow, follows actual route progress, and physically settles without pose snaps. The electric step-through retains independent wheels, handlebars, lean, stand and safe stationary dismounting. The forest/gravel rally retains bends, hills, rocks, stage splits and brake-only S. Circular steering wheels spin on fixed tilted shafts; right input is clockwise from the actual seated camera. Vehicles share live obstacles and ignore only their own identities; no driving-school signs, cones or exam objectives.
- **Smart home and atmosphere:** P's terminal provides per-room/all lighting, fireplace/aquarium switches, day/evening/night, clear/rain weather, look sensitivity, snooker guides and sound on/off. Asset-free ambience and vehicle sounds start only after an eligible trusted gesture; mute, blur, hidden documents, shell pauses and stop silence all live/scheduled audio immediately. `villaInteractionAudio.ts` owns distinct semantic cues for accepted/rejected physical and UI actions, with one dispatch across keyboard/native Use paths; no-op selections stay silent. Use actual tea/lift/jump/collision transitions, not per-frame success sounds. Local stream, faucet and elevator loops allocate only while audible; rally and higher-speed road engines normalize to their real limits. Preserve the bounded 24 one-shot sources and all-loop lifecycle cleanup. Electric vehicles must not have combustion idling, and upstairs listeners must not hear the downstairs fireplace as if on its hearth. CCTV points at the current garage/pool datums, not their former locations. Time, weather and light levels ease gradually; night remains readable with the lights off. Room lamps, corridors, roof LEDs and garden fixtures agree with their switches while reusing a six-point-light physical budget. CCTV shows one selected, genuinely rendered local-camera view at a time—not a decorative still or many simultaneous scene passes. Sensitivity applies to mouse/touch look without changing movement speed. This is game functionality, not a replacement shell or persistent dashboard.
