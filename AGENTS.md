...
**Last reconciled with production:** 2026-09-21 (CS aiming-feedback repair). CS `1.1.2` at `a730c5e13ebddac9d0b53688b3f8111938be7ded` (API 1, target generation 5, sequence 235001) passed exact-SHA Actions run `35617484160`. Public identity, content types and SHA-256 inventories passed for all 169 current CS files, 169 retained CS `1.1.1` files at `fc0a1d8cea6a805c8875839a1a25b858048fe267`, and 10 files of the unchanged pinned shell `0.4.0` at `831408bdee7070d459bf0190e4db8154ded6da0c` (shell generation 3). Missing game/shell resources returned 404. The normal live shell entered a match with nine animated operators and no page errors. Game-owned isolated real-Snow fixtures loaded the actual live release and verified body/head/kill feedback, scopes, paint order and paused resize at desktop/DPR 1 and notched landscape/DPR 2; their deterministic actor placement is not a full-map combat playtest. Catalog generation is 94 with the same 9 active IDs (`aimlab`, `cs`, `cs-kimi`, `gacha`, `minesweeper`, `parking`, `sudoku`, `texashold`, `villa`); all eight non-CS entries and all 19 delist tombstones are unchanged. No other game or shell was republished or independently gameplay-reverified in this pass. The earlier delist operations advanced catalog generation 73→89→92, left each removed game's tombstone at generation 3/sequence 202002, and retained immutable files; a future runtime publish can legitimately re-list a delisted game. Villa remains `1.10.0` at `da8256e0464256636f739575538fb64527b69100`. This line is a claim like any other: re-check live `/games/index.json` and `/manifest.json` before trusting it, and update it whenever a statement here is verified or corrected.
