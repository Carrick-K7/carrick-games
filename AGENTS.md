...
**Last reconciled with production:** 2026-09-20 (fourth pass, CS UI repair). CS `1.1.1` at `fc0a1d8cea6a805c8875839a1a25b858048fe267` (API 1, target generation 4, sequence 233001) passed exact-SHA Actions run `35516915933`. Public identity, content types and SHA-256 inventories passed for all 169 current CS files, 169 retained CS `1.1.0` files at `7b2e355e3854f1f33034ca459501bfb3ec299182`, and 10 files of the unchanged pinned shell `0.4.0` at `831408bdee7070d459bf0190e4db8154ded6da0c` (shell generation 3). Missing game/shell resources returned 404. Live Chromium desktop (1100×640) and touch phone (320×568) verified reachable Team DM Start actions, isolated settings, nine animated bots and no page/HTTP errors. Catalog generation is 93 with the same 9 active IDs (`aimlab`, `cs`, `cs-kimi`, `gacha`, `minesweeper`, `parking`, `sudoku`, `texashold`, `villa`); all eight non-CS entries and all 19 delist tombstones are unchanged. No other game or shell was republished or independently gameplay-reverified in this pass. The prior delist operations advanced catalog generation 73→89→92, left each removed game's tombstone at generation 3/sequence 202002, and retained immutable files; a future runtime publish can legitimately re-list a delisted game. Villa remains `1.10.0` at `da8256e0464256636f739575538fb64527b69100`. This line is a claim like any other: re-check live `/games/index.json` and `/manifest.json` before trusting it, and update it whenever a statement here is verified or corrected.
