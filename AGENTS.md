...
- Targeted validation walks the selected workspace and its **transitively declared libraries**, not unrelated game source or `game.json`. A malformed unrelated game cannot block a targeted game build/typecheck. Root workspace, shared toolchain or external lock changes may deliberately cause conservative fan-out; a broken root install is not independent-game isolation. Because the root `package.json` is a runtime input for every component, even adding one root devDependency makes all 28 games and the shell runtime-affected: every component then needs a higher SemVer in the same push, or every cell fails its version check and nothing publishes. Prefer a component-local `devDependencies` entry over a root one.
