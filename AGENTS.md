...
**Last reconciled with production:** 2026-09-20 (third pass, delist operations). Nineteen games are delisted from `/games/index.json` via fenced `remove-game`: the sixteen from the second pass plus `doodlejump`, `snake` and `2048` (each `tombstones[id]` at generation 3, sequence 202002; catalog generation 73→89→92, shell generation unchanged at 3). The 9 remaining entries (`2048` removed; now `aimlab`, `cs`, `cs-kimi`, `gacha`, `minesweeper`, `parking`, `sudoku`, `texashold`, `villa`) are byte-identical to the prior catalog; the shell is unchanged at `0.4.0` @ `831408bdee7070d459bf0190e4db8154ded6da0c`; removed games' immutable files still return 200 with correct content types (retention/rollback intact). A future runtime publish of a delisted game legitimately wins the sequence fence and re-lists it. Earlier same-day reconciliation: CS `1.1.0` at `7b2e355e3854f1f33034ca459501bfb3ec299182` with pinned shell `0.4.0`, Actions run `35509293470`; Villa `1.10.0` at `da8256e0464256636f739575538fb64527b69100`. This line is a claim like any other: re-check live `/games/index.json` and `/manifest.json` before trusting it, and update it whenever a statement here is verified or corrected.
