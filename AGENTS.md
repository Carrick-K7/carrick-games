...
- Minimize external runtime dependencies. The standing exception: shooter-family games (currently `counterstrike` via `src/games/counterstrikeScene3d.ts`) and the user-requested Warm Villa (`villa`) exploration experience may use the existing `three` dependency for real-3D rendering, rendered into the game's 2D canvas so shell contracts stay intact. Any other new runtime dependency still requires explicit user approval, must be declared in `package.json`, and must be listed here when added.
