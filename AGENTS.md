...
**Last reconciled with production:** 2026-09-22 (CS tactical UI and automatic reload). CS `1.2.0` at `b1900e0f38c8e07cc7478a028bc7bae7a0f9f4fb` (API 1, target generation 6, sequence 237001) passed exact-SHA Actions run `35796212088`. Public identity, content types and SHA-256 inventories passed for all 189 current CS files, 169 retained CS `1.1.2` files at `a730c5e13ebddac9d0b53688b3f8111938be7ded`, and 10 files of the unchanged pinned shell `0.4.0` at `831408bdee7070d459bf0190e4db8154ded6da0c` (shell generation 3). Missing game/shell resources returned 404. The normal live shell verified genuine map cards, stable hover targets, arena entry and new HUD artwork. Game-owned controlled real-Snow fixtures loaded the actual live release to verify body/head/kill feedback, scopes, paint order and paused resize, plus automatic timed reload after the last shot without another input, correct reserve accounting, zero-reserve behavior and the compact reload indicator. These isolated deterministic fixtures are not a full-map combat playtest. Catalog generation is 95 with the same 9 active IDs (`aimlab`, `cs`, `cs-kimi`, `gacha`, `minesweeper`, `parking`, `sudoku`, `texashold`, `villa`); all eight non-CS entries and all 19 delist tombstones are unchanged. No other game or shell was republished or independently gameplay-reverified in this pass. The earlier delist operations advanced catalog generation 73→89→92, left each removed game's tombstone at generation 3/sequence 202002, and retained immutable files; a future runtime publish can legitimately re-list a delisted game. Villa remains `1.10.0` at `da8256e0464256636f739575538fb64527b69100`. This line is a claim like any other: re-check live `/games/index.json` and `/manifest.json` before trusting it, and update it whenever a statement here is verified or corrected.
...
- Exactly three permanent **real 44px-high shell targets**: the brand mark at 44×44 (top/left 12px plus safe area for fixed games; right 116px for CS/CS Kimi/Villa), help at right 64px and menu at right 12px. Preserve the exact 22px teal gamepad. **No brand text is persistent**: the full stacked "Carrick Games" wordmark appears only on the `…` menu and the library, while the mark keeps its accessible name. All hide during pointer capture; responsive HUDs reserve the existing top-right utility cluster and 64px content clearance.
