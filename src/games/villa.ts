...
export class VillaGame extends BaseGame {
  ...
  private setTerminal(open: boolean) {
    ...
    this.syncUseButton(); this.publishState();
  }
  ...
  private compactHud() {
    ...
    return this.touchMode || this.width - safe.left - safe.right < 1080 || this.height - safe.top - safe.bottom < 600;
  }
  ...
  private buttons(): Button[] {
    ...
    const entries: Array<{ id: string; label: string; short: string }> = [
      { id: 'terminal', label: zh ? 'P  智能终端' : 'P  Smart home', short: zh ? '终端' : 'P' },
      { id: 'map', label: zh ? 'M  导览图' : 'M  Floor plan', short: zh ? '图' : 'M' },
      { id: 'time', label: this.state.home.timeOfDay === 'day' ? (zh ? 'T  傍晚' : 'T  Evening') : this.state.home.timeOfDay === 'evening' ? (zh ? 'T  夜晚' : 'T  Night') : (zh ? 'T  白天' : 'T  Day'), short: this.state.home.timeOfDay === 'night' ? '☀' : '☾' },
  ...
  draw(ctx: CanvasRenderingContext2D) {
    ...
    if (zh !== this.lastLang) {
      this.lastLang = zh;
      this.canvas.setAttribute('aria-label', zh ? `暖居别墅 ${VILLA_VERSION}。坐北朝南的 3D 漫游。WASD 移动，E 靠近互动，P 智能终端，M 导览图，? 查看完整操作。` : `Warm Villa ${VILLA_VERSION}. A south-facing 3D home. WASD moves, E interacts nearby, P opens the smart terminal, M opens the map, ? shows all controls.`);
    }
