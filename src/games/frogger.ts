...
type GameState = 'waiting' | 'playing' | 'gameover' | 'levelcomplete';
...
interface MovingObj {
  ...
  w: number;
  h: number;
  speed: number; // px/s, positive = right
}
...
export class FroggerGame extends BaseGame {
  ...
  private frogCol = 0;
  private frogRow = 0;
  private frogDir: Direction = 'UP';
  private hopPhase = 0; // 0=idle, >0=hopping (0..1)
  private hopStartX = 0;
  private hopStartY = 0;
  private hopTargetX = 0;
  private hopTargetY = 0;
  private hopDur = 0.12; // seconds
  private isDead = false;
  private deathTimer = 0;
  private deathDur = 0.8;
  ...
  private homesFilled = 0;

  // Level complete timer
  private levelCompleteTimer = 0;
  private levelCompleteDur = 1.5;
  ...
  private buildLanes() {
    ...
    for (let r = 1; r <= 5; r++) {
      const lane: Lane = { type: 'water', y: HUD_H + r * GRID, objects: [] };
    ...
    for (let r = 7; r <= 11; r++) {
      const lane: Lane = { type: 'road', y: HUD_H + r * GRID, objects: [] };
      const dir = r % 2 === 0 ? -1 : 1;
  ...
  private tryHop(dir: Direction) {
    if (this.state !== 'playing' || this.isDead || this.hopPhase > 0) return;
  ...
  update(dt: number) {
    ...
    for (const lane of this.lanes) {
      for (const obj of lane.objects) {
        obj.x += obj.speed * dt;
        if (obj.speed > 0 && obj.x > CANVAS_W) obj.x = -obj.w;
        if (obj.speed < 0 && obj.x + obj.w < 0) obj.x = CANVAS_W;
      }
    }
    ...
    if (this.frogRow >= 1 && this.frogRow <= 5) {
      ...
      for (const obj of lane.objects) {
        if (this.rectsOverlap(
          this.frogX + 4, this.frogY + 4, FROG_W - 8, FROG_H - 8,
  ...
  private drawFrogShape(ctx: CanvasRenderingContext2D, cx: number, cy: number, isDark: boolean, small: boolean) {
    const r = small ? 10 : 12;
  ...
  private drawOverlay(ctx: CanvasRenderingContext2D, isDark: boolean, en: string, zh: string) {
