import { PLAYER_RADIUS, POOL, STAIR_HOLE, VILLA_WALL_COLLIDERS, VILLA_RAILS, villaSupportAt, type VillaCollider, type VillaPosition } from './villaWorld.js';
...
export const VILLA_SCOOTER_LIMITS = { halfWidth: .6, halfLength: 1.03, height: 1.8, wheelbase: 1.3, wheelRadius: .235, maxSpeed: 6.1, maxSteer: .5 };
export const VILLA_SCOOTER_BOUNDS = { minX: -24.5, maxX: 27.5, minZ: -16.5, maxZ: 56.5 };
...
const transform = (p: VillaScooterPose, x: number, z: number): VillaPosition => ({ x: p.x + Math.cos(p.yaw) * x + Math.sin(p.yaw) * z, y: 0, z: p.z - Math.sin(p.yaw) * x + Math.cos(p.yaw) * z });
...
export function villaScooterAnchors(pose: VillaScooterPose) {
  const corners = villaScooterFootprint(pose), exits = [transform(pose, 1, -.23), transform(pose, -1, -.23)];
  return { seat: transform(pose, 0, -.23), exit: exits[0]!, exits, interaction: exits[0]!,
    body: { minX: Math.min(...corners.map(p => p.x)), maxX: Math.max(...corners.map(p => p.x)), minZ: Math.min(...corners.map(p => p.z)), maxZ: Math.max(...corners.map(p => p.z)), minY: 0, maxY: 1.4 } satisfies VillaCollider };
}
...
export function villaScooterOverlaps(pose: VillaScooterPose, box: VillaCollider): boolean {
  if (isVillaScooterCollider(box) || box.maxY <= .025 || box.minY >= VILLA_SCOOTER_LIMITS.height) return false;
...
function groundClear(x: number, z: number, radius = 0): boolean {
  const bounds = VILLA_SCOOTER_BOUNDS;
  if (x - radius < bounds.minX || x + radius > bounds.maxX || z - radius < bounds.minZ || z + radius > bounds.maxZ) return false;
  const probes = radius === 0 ? [[0, 0]] : [[0, 0], [-radius, -radius], [-radius, radius], [radius, -radius], [radius, radius]];
  return probes.every(([dx, dz]) => {
    const support = villaSupportAt(x + dx!, z + dz!, 0, VILLA_SCOOTER_LIMITS.height);
    return support !== null && Math.abs(support) < .001;
  });
}
export function villaScooterPoseBlocked(pose: VillaScooterPose, obstacles: readonly VillaCollider[]): boolean {
  ...
  if (!groundClear(pose.x, pose.z) || villaScooterFootprint(pose).some(p => !groundClear(p.x, p.z))) return true;
...
export function villaScooterExitClear(pose: VillaScooterPose, obstacles: readonly VillaCollider[], side: 1 | -1 = 1): boolean {
  ...
  for (const list of [VILLA_SCOOTER_FIXED_COLLIDERS, VILLA_WALL_COLLIDERS, VILLA_RAILS, obstacles]) for (const box of list) {
    if (!isVillaScooterCollider(box) && box.maxY > .025 && box.minY < 1.8 && routeTouches(anchors.seat, exit, box)) return false;
  }
  for (let i = 0; i <= 40; i++) {
...
/** W=throttle +1; S=throttle -1 OR brake=true is the service brake.
 * Space=handbrake=true. No reverse. Positive D/right DECREASES world yaw.
...
export function advanceVillaScooter(state: VillaScooterState, input: VillaScooterInput, dt: number, obstacles: readonly VillaCollider[]): void {
  ...
  state.speed = clamp(finite(state.speed), 0, VILLA_SCOOTER_LIMITS.maxSpeed);
  ...
  for (let i = 0; i < count; i++) {
    ...
    const acceleration = state.handbrake ? -6 : input.brake || throttle < 0 ? -4.6 : throttle * 1.9 - .22 - state.speed * .09;
    state.speed = clamp(state.speed + acceleration * h, 0, VILLA_SCOOTER_LIMITS.maxSpeed);
    // Reduce steering lock at speed and constrain lean in the model; no wheelies.
    const angle = -state.speed / VILLA_SCOOTER_LIMITS.wheelbase * Math.tan(state.steering) * h / (1 + state.speed * .13);
