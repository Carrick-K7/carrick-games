 ...
 *
 * drawWeaponIcon(ctx, iconId, cx, cy, opts) renders one orthographic front
 * view — the 正视图 (principal view) of an engineering three-view drawing
 * (三视图): the flat profile facing the viewer square-on, muzzle pointing
 * +x, magazines and grips hanging -y. No three-quarter camera angle and no
 * perspective foreshortening. Multi-tone steel, wood, polymer, mounts,
 * sights, magazines, and controls keep each weapon recognizable while
 * preserving a clean outer silhouette. Coordinates are normalized to
 * -0.5..0.5 (gun points +x). Every profile is supersampled into a cached
 * offscreen canvas and smoothly reduced, so the same art stays crisp from
 * tiny reel thumbnails up to large result cards.
 *
