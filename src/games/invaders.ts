...
export class InvadersGame extends BaseGame {
  ...
  private bulletHeight = 10;
  private bulletSpeed = 420;

  private enemyRows = 5;
  private enemyCols = 8;
  private enemyWidth = 28;
  private enemyHeight = 20;
  private enemyGapX = 12;
  private enemyGapY = 14;
  private enemies: Invader[] = [];
  private enemyDir = 1; // 1 = right, -1 = left
  private enemySpeed = 30;
  private enemyBaseY = 40;
  private enemyMoveTimer = 0;
  private enemyMoveInterval = 0.5;
  private enemyStepX = 6;
  private enemyStepY = 10;
  private enemyBullets: EnemyBullet[] = [];
  private enemyBulletSpeed = 160;
  private enemyFireTimer = 0;
  private enemyFireInterval = 1.2;
  ...
  private get aliveEnemies(): Invader[] {
  ...
  update(dt: number) {
    ...
    for (const b of this.bullets) {
      ...
      if (b.y + this.bulletHeight < 0) b.active = false;
    }
  ...
  draw(ctx: CanvasRenderingContext2D) {
    ...
    for (const e of this.enemies) {
      ...
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.moveTo(e.x + 2, e.y + 4); ctx.lineTo(e.x + 2 + wiggle, e.y - 2);
      ctx.moveTo(e.x + this.enemyWidth - 2, e.y + 4); ctx.lineTo(e.x + this.enemyWidth - 2 - wiggle, e.y - 2);
      ctx.stroke();
    }
