...
export function renderLevelGridHTML(state: LevelSelectState, selectedLevel: number, zh: boolean): string {
  ...
  for (let i = 0; i < state.totalLevels; i++) {
    ...
    let badge = '';
    if (locked) badge = '<span class="lc-lock" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round"><rect x="5" y="11" width="14" height="9" rx="2"/><path d="M8 11V7a4 4 0 0 1 8 0v4"/></svg></span>';
