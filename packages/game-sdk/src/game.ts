...
import { releaseResource } from './catalog.js';
...
export interface GamePresentation {
  ...
  /** Actions belong to this host instance and are discarded when it leaves. */
  setActions?(actions: readonly GameMenuAction[]): void;
}
...
export interface Game {
  ...
  /** Optional JSON-safe diagnostics; only the game knows its renderer internals. */
...
export interface GameHost {
  ...
  /** Resolve a game-owned path against this immutable release, never a global base. */
...
export function createDefaultGameHost(
  ...
  // Isolated browser fixtures may construct the class exported by the active
  // release. Capture its canvas context ONCE; later game switches cannot retarget it.
  const assetBase = canvas.dataset?.gameAssetBase;
  const origin = assetBase ? window.location.origin : undefined;
  return {
    canvas,
    logicalWidth,
    logicalHeight,
    assetUrl: assetBase && origin ? path => releaseResource(path, assetBase, origin) : undefined,
    isDarkTheme,
    isZhLang,
    isPixelMode,
