...
import { getRetroPalette, type RetroPalette } from '@carrick/game-sdk/render';
import {
  Particles,
  ScreenShake,
  FloatTexts,
...
export class AsteroidsGame extends BaseGame {
  private ship: { x: number; y: number; vx: number; vy: number; angle: number; alive: boolean; blinkTimer: number; invincible: boolean; invTimer: number } = { x: 0, y: 0, vx: 0, vy: 0, angle: 0, alive: true, blinkTimer: 0, invincible: true, invTimer: 3 };
  ...
  private wrap(v: Vec, margin = 20): Vec {
    if (v.x < -margin) v.x = this.width + margin;
    if (v.x > this.width + margin) v.x = -margin;
    if (v.y < -margin) v.y = this.height + margin;
    if (v.y > this.height + margin) v.y = -margin;
    return v;
  }

  private makeAsteroid(x: number, y: number, size: number, inheritVx?: number, inheritVy?: number): Asteroid {
  ...
  update(dt: number) {
    ...
    if (this.ship.alive) {
      // Rotation
      if (this.rotateLeft) this.ship.angle -= 4 * dt;
      if (this.rotateRight) this.ship.angle += 4 * dt;
      // Thrust
      if (this.thrust) {
        const thrust = 250;
        ...
        if (Math.hypot(dx, dy) < a.radius) {
          ...
          if (a.size < 2) {
            ...
            for (let i = 0; i < count; i++) {
              ...
              const child = this.makeAsteroid(
                a.x + Math.cos(angle) * 10,
                a.y + Math.sin(angle) * 10,
                a.size + 1,
                a.vx,
                a.vy,
              );
    ...
    if (this.saucer) {
      ...
      if (s.y < 30) s.y = 30;
      if (s.y > this.height - 30) s.y = this.height - 30;
  ...
  handleInput(e: KeyboardEvent | TouchEvent | MouseEvent) {
    ...
    if (e instanceof TouchEvent) {
      ...
      if (e.type === 'touchstart') {
        ...
        this.rotateRight = false;
        this.thrust = false;
        if (cx < -60) this.rotateLeft = true;
        else if (cx > 60) this.rotateRight = true;
        if (cy < -60) this.thrust = true;
        this.shootPending = true;
