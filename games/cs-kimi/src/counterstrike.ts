...
export class CounterStrikeGame extends BaseGame {
  ...
  init() {
    ...
    this.startRound(1);
    this.syncDebugState();
    this.registerCleanup(installGameDebug('__CS_DEBUG__', {
      look: (angle: number, pitch = 0) => { this.angle = angle; this.pitch = pitch; },
      tp: (x: number, y: number) => {
        // Fighter position is authoritative; the camera follows it.
      ...
      info: () => {
        ...
        return { weapon: weapon?.def.id ?? null, mag: weapon?.mag ?? -1, slot: player.slot, phase: this.phase, x: player.x, y: player.y };
      },
      shoot: () => { const player = this.fighters[0]; if (player?.alive) this.fireShot(player, this.angle); },
      give: (weaponId: string) => {
        const player = this.fighters[0];
        if (!player || !Object.prototype.hasOwnProperty.call(WEAPONS, weaponId)) return;
        player.primary = this.makeWeapon(weaponId as keyof typeof WEAPONS);
        player.slot = 'primary';
      },
    }));
  }
  ...
  getDiagnostics() {
    return {
      cameraAspect: this.scene3d?.cameraAspect,
      renderWidth: this.scene3d?.canvas?.width,
      renderHeight: this.scene3d?.canvas?.height,
      buyOpen: this.buyOpen, paused: this.paused,
      raycastWidth: this.rw, raycastHeight: this.rh,
      projectionHalfFov: this.halfFovTan(),
    };
  }
