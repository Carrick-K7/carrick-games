...
test('villa garden: native coarse taps feed a pet, switch the faucet and restore quiet controls', async ({ browser }) => {
  ...
  try {
    ...
    const terminalPoint = await page.evaluate(() => { const g = (window as any).garden.game, b = g.buttons().find((b: any) => b.id === 'terminal'), r = g.canvas.getBoundingClientRect();
      return { x: r.x + (b.x + b.w / 2) * r.width / g.width, y: r.y + (b.y + b.h / 2) * r.height / g.height }; });
    await page.touchscreen.tap(terminalPoint.x, terminalPoint.y);
    // This isolated host has no shell actions: use the native Settings fallback.
    ...
    const restore = await page.evaluate(() => { const g = (window as any).garden.game, b = g.buttons().find((b: any) => b.id === 'terminal'), r = g.canvas.getBoundingClientRect();
      return { x: r.x + (b.x + b.w / 2) * r.width / g.width, y: r.y + (b.y + b.h / 2) * r.height / g.height }; });
    // Opening Terminal leaves immersive mode; close it before using the scene.
    await page.touchscreen.tap(restore.x, restore.y);
    await terminal.locator('[data-villa-terminal-close]').tap(); await use();
    expect(await page.evaluate(() => (window as any).garden.game.canvas.dataset.villaFaucet)).toBe('on');
