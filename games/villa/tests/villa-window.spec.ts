...
test.describe('Villa’s direct help and browser-owned game window', () => {
  ...
  test('the retired F shortcut never requests fullscreen, even when its API rejects', async ({ page }) => {
    ...
    // Browser-owned F11 may be explained in notes, never offered as a game action.
    const actionRows = page.locator('#keyboardPanel .input-map-row');
    await expect(actionRows).not.toHaveCount(0);
    await expect(actionRows.filter({ hasText: /fullscreen|全屏/i })).toHaveCount(0);
    await page.locator('#helpBtn').click();
    await expect(canvas).toBeFocused();
