...
test('villa coarse-pointer map and real multi-touch use usable targets and independent fingers', async ({ browser }) => {
  ...
  const geometry = await canvas.evaluate((c: HTMLCanvasElement) => {
    ...
    const map = (JSON.parse(c.dataset.villaUtilities || '[]') as Array<{ id: string; x: number; y: number; w: number; h: number }>)
      .find(button => button.id === 'map');
    if (!map) throw new Error('Villa location/map target is unavailable');
    return {
      // The location badge is the game's direct map target.
      map: client(map.x + map.w / 2, map.y + map.h / 2),
  ...
  await page.locator('#currentGameActions [data-action-id="villa-home"]').tap();
