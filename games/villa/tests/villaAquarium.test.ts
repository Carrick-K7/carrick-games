...
describe('locally authored top-view medaka', () => {
  ...
  it('keeps ten fish and bubbles inside the tank, even when fed/reset or given nonfinite clocks, and ships no shrimp', () => {
    ...
    expect(root.getObjectByName('aquarium/shrimp-1')).toBeUndefined();
    expect(Object.keys(VILLA_AQUARIUM_LIFE)).not.toContain('shrimp');
  ...
  it('batches animated parts conservatively and preserves scene-owned resources across updates', () => {
    ...
    const initial = (root.getObjectByName('aquarium/medaka-tail') as THREE.InstancedMesh).instanceMatrix.array.slice();
    life.update(2.3, 0.1, false); expect((root.getObjectByName('aquarium/medaka-tail') as THREE.InstancedMesh).instanceMatrix.array).not.toEqual(initial);
