...
async function mount(page: Page, touch = false) {
  ...
  await page.evaluate(async ({ url, touch }) => {
    ...
    // This fixture raises its canvas above Snake; its native modal must stay above it.
    if (touch) game.terminal.element.style.zIndex = '10002';
...
test('villa life: immersive HUD keeps only location/map and terminal; an occupied lift stays open until departure', async ({ page }) => {
  test.setTimeout(90_000); await mount(page);
  try {
    const result = await page.evaluate(() => {
      ...
      const utilityIds = game.buttons().map((button: any) => button.id);
      press('i'); const normalText = hudText(), restored = game.canvas.dataset.villaImmersive === 'false' && game.buttons().some((b: any) => b.id === 'map');
      const normalUtilityIds = game.utilityButtons().map((button: any) => button.id);
      ...
      return { immersiveText, immersive, promptHidden, utilityIds, normalText, normalUtilityIds, footerText, restored, heldOpen, beforeTimeout, noTravel, closed, reopened };
    });
    expect(result.immersive).toBe('true'); expect(result.utilityIds).toEqual(['map', 'terminal']);
    expect(result.immersiveText).toEqual(['1F · Welcome home ▾', 'Terminal']);
    expect(result.normalUtilityIds).toEqual(['map', 'terminal']);
    expect(result.normalText.length).toBeGreaterThan(1); expect(result.normalText).toContain('1F · Welcome home ▾');
    expect(result.normalText).toContain('Terminal');
    expect(result.footerText).toEqual([]); // No persistent shortcut footer or duplicate map/time/home controls.
    for (const flag of ['promptHidden', 'restored', 'heldOpen', 'beforeTimeout', 'noTravel', 'closed', 'reopened'] as const) expect(result[flag], flag).toBe(true);
  } finally { await dispose(page); }
});
...
test('villa life: coarse taps operate crouch/jump and snooker buttons with scaled coordinates and cancellation', async ({ browser }) => {
  ...
  try {
    ...
    await tap('terminal');
    // This isolated host exposes menu actions through the native Settings fallback.
    const terminal = page.locator('div[data-villa-terminal]');
    await terminal.locator('[data-villa-terminal-tab="settings"]').tap();
    await terminal.locator('[data-villa-fallback-action="villa-immersive"]').tap();
    expect(await page.evaluate(() => (window as any).villaLife.game.canvas.dataset.villaImmersive)).toBe('true');
    await tap('terminal');
    await terminal.locator('[data-villa-terminal-close]').tap();
