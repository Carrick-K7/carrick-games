...
export class WordleGame extends BaseGame {
  ...
  private evaluateGuess(guess: string): TileState[] {
    const result: TileState[] = Array(COLS).fill('gray');
  ...
  private drawGrid(ctx: CanvasRenderingContext2D, theme: Theme) {
  ...
  private drawKeyboard(ctx: CanvasRenderingContext2D, theme: Theme) {
    this.keyRects = [];
    const baseWidth = 32;
    const wideWidth = 48;
  ...
  private drawKey(
    ...
    let label = key;
    if (key === 'Enter') label = 'ENT';
    if (key === 'Backspace') label = 'DEL';
