...
import { GachaNavigationHud, GachaProgressHud, type GachaNavButton } from './gachaProgress.js';
import {
  GACHA_POOL,
  GACHA_TIERS,
  GACHA_TIER_ORDER,
...
export class GachaGame extends BaseGame {
  ...
  private readonly navigationHud = new GachaNavigationHud();
  /**
  ...
  /** Resolve the responsive aspect locally before the base class fits it. */
  override setViewport(viewport: GameViewport) {
    if (![viewport.width, viewport.height].every(n => Number.isFinite(n) && n > 0)) return;
    const portrait = viewport.width < 700 && viewport.height > viewport.width * 1.05;
    const aspect = portrait ? 4 / 5 : 4 / 3;
  ...
  override setPresentationPaused(paused: boolean) {
    ...
    if (changed && paused) this.sfx.spinStop();
    else if (changed && this.running && this.screen === 'opening' && this.sfx.enabled) this.sfx.spinStart();
    this.syncNavigation();
  }

  destroy() {
    this.navigationHud.destroy();
  ...
  /* ─── Navigation and host-owned secondary actions ─── */

  private get opening(): boolean {
    return this.screen === 'unlock' || this.screen === 'opening';
  }

  private publishMenuActions() {
    this.host.presentation?.setActions?.([
      { id: 'gacha-stats', label: 'Stats', labelZh: '抽取统计', disabled: this.opening, run: () => this.openStats() },
      { id: 'gacha-sound', label: 'Sound', labelZh: '音效', checked: this.sfx.enabled, run: () => this.toggleSound() },
    ]);
  }
  ...
  private toggleSound() {
    ...
    if (this.sfx.enabled) {
      this.sfx.prime();
      if (this.screen === 'opening' && !this.presentationPaused) this.sfx.spinStart();
    } else this.sfx.spinStop();
    this.publishMenuActions();
  }

  private openStats() {
    // A stale menu callback must not interrupt or restart the committed reveal.
  ...
  private openCollection() {
    this.sfx.spinStop();
    this.sfx.click();
    // Deliberate browsing can bail mid-spin; the pull was already recorded once.
    this.animMode = null;
  ...
  /** CSS geometry below the existing top band, independent of canvas fit/DPR. */
  private navigationBox() {
    if (this.viewport) {
      const { top, right, bottom, left } = this.viewport.safeArea;
      return { x: left, y: top, w: this.viewport.width - left - right, h: this.viewport.height - top - bottom };
    }
    // Legacy setDisplayScale hosts have no viewport; these are SDK-written sizes.
    return {
      x: parseFloat(this.canvas.style.left) || 0, y: parseFloat(this.canvas.style.top) || 0,
      w: parseFloat(this.canvas.style.width) || this.width, h: parseFloat(this.canvas.style.height) || this.height,
    };
  }

  private navButtons(): GachaNavButton[] {
    const { w, h } = this.navigationBox();
    const back: GachaNavButton = {
      id: 'back', label: this.isZhLang() ? '返回抽卡' : 'Back to Gacha',
      x: w - 144, y: 8, w: 132, h: 44,
      run: () => { if (!this.presentationPaused) { this.sfx.click(); this.gotoMenu(); } },
    };
    if (this.screen === 'gallery' || this.screen === 'stats') return [back];
    const collection: GachaNavButton = {
      id: 'collection', label: this.isZhLang() ? '收藏册' : 'Collection',
      x: w - 116, y: 8, w: 104, h: 44,
      run: () => { if (!this.presentationPaused) this.openCollection(); },
    };
    if (!this.opening) return [collection];
    // Keep the centered opening caption clear, even in a height-constrained fit.
    collection.y = h - 52;
    back.x = 12;
    back.y = h - (w < 272 ? 104 : 52);
    return [collection, back];
  }

  private syncNavigation() {
    this.canvas.dataset.gachaScreen = this.screen;
    this.navigationHud.update(this.canvas, this.navigationBox(), this.navButtons(), this.presentationPaused, this.isZhLang());
  }
  ...
  handleInput(e: KeyboardEvent | TouchEvent | MouseEvent) {
    ...
    if (e instanceof KeyboardEvent) {
      if (this.navigationHud.ownsTarget(e.target) && (e.key === ' ' || e.key === 'Enter')) return;
      ...
      // Keyboard access also works for isolated hosts without presentation actions.
      if (e.key === 's' || e.key === 'S') {
        this.openStats();
        return;
      }
    ...
    if (e instanceof MouseEvent && e.type === 'mousemove') {
      ...
      const clickable = overRail || this.screen === 'menu' || this.screen === 'result';
